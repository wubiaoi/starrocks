...
public:
    GetHdfsFileReadOnlyHandle(const FSOptions options, std::string namenode, std::string path, int buffer_size)
            : _options(std::move(options)), _path(std::move(path)), _namenode(std::move(namenode)), _buffer_size(buffer_size) {}

    StatusOr<hdfsFS> getOrCreateFS() {
        if (_hdfs_client == nullptr) {
            SCOPED_RAW_TIMER(&_open_time_ns);
            RETURN_IF_ERROR(HdfsFsCache::instance()->get_connection(_namenode, _hdfs_client, _options));
        }
    ...
    const std::string& getNamenode() const { return _namenode; }
...
StatusOr<int64_t> HdfsInputStream::read(void* data, int64_t size) {
    ...
    if (!_seek) {
        int ret = hdfsSeek(_handle->getFS(), file, _offset);
        if (ret == -1) {
            return Status::IOError(fmt::format("fail to hdfdSeek {}: {}", _handle->getPath(), get_hdfs_err_msg()));
        }
        _seek = true;
    }
    ...
    if (_file_size == 0) {
        auto ret = call_hdfs_scan_function_in_pthread([this] {
            ...
            if (UNLIKELY(info == nullptr)) {
                return Status::IOError(fmt::format("Fail to get path info of {}: {}", _handle->getPath(), get_hdfs_err_msg()));
            }
...
StatusOr<std::unique_ptr<SequentialFile>> HdfsFileSystem::new_sequential_file(const SequentialFileOptions& opts,
    ...
    auto handle = std::make_unique<GetHdfsFileReadOnlyHandle>(_options, namenode, relative_path, hdfs_read_buffer_size);
